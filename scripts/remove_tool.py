...
def check_tool_usage(tool_name: str) -> List[str]:
    ...
    if config_files:
        print(f"⚠ Warning: {tool_name} is referenced in {len(config_files)} config file(s):")
...
def remove_from_gitignore(tool_name: str, dry_run: bool = False) -> None:
    ...
    if not gitignore_path.exists():
        print(f"✓ .gitignore file not found, nothing to remove")
        return
...
def remove_submodule(tool_name: str, tool_path: Path, dry_run: bool = False) -> None:
    ...
    success, output = run_command(
        ["git", "submodule", "deinit", "-f", str(tool_path)],
        capture_output=True
    )
    if not success:
...
def interactive_confirmation(tool_name: str, config_files: List[str], dependent_tools: List[str]) -> bool:
    ...
    response = input(f"\nAre you sure you want to remove '{tool_name}'? (yes/no): ").lower().strip()
...
This utility helps manage tool lifecycle and switch between different implementations.
    ...
    try:
        ...
        if not args.force and not args.dry_run:
            if not interactive_confirmation(args.tool_name, config_files, dependent_tools):
        ...
        if args.dry_run:
            ...
            print("No changes were made. Use without --dry-run to actually remove the tool.")
        else:
        ...
        print("2. Consider adding a replacement tool: python scripts/plugin.py <new_tool> <repo_url>")
