...
def run_command(cmd: list, cwd: Optional[Path] = None, capture_output: bool = False, 
                check: bool = True, timeout: int = 300) -> tuple[bool, str]:
    """
...
def check_prerequisites() -> None:
    ...
    if not success:
        raise PluginError("Not in a git repository. Please run this from your experimentStash root.")
    print("✓ In git repository")


def clone_and_add_submodule(tool_name: str, github_url: str, branch: str = "main") -> Path:
    ...
    if not success and "A git directory for" in output and "use the '--force' option" in output:
        print(f"🔄 Retrying with --force option...")
        cmd = ["git", "submodule", "add", "--force", "-b", branch, github_url, str(tool_path)]
        success, output = run_command(cmd, capture_output=True)
    ...
    if not success and "already exists in the index" in output:
        print(f"🔄 Removing from index and retrying...")
        run_command(["git", "rm", "-r", "--cached", str(tool_path)], capture_output=True)
        run_command(["git", "reset", "HEAD", str(tool_path)], capture_output=True)
        cmd = ["git", "submodule", "add", "-b", branch, github_url, str(tool_path)]
        success, output = run_command(cmd, capture_output=True)
...
def update_meta_yaml(tool_name: str, tool_path: Path, entrypoint: str = "src/main.py") -> None:
...
def print_success_summary(tool_name: str, tool_path: Path) -> None:
    ...
    print(f"✓ Environment: uv-based")
    print(f"✓ Metadata: Updated in configs/meta.yaml")
    print(f"✓ Gitignore: Added to .gitignore (use --no-gitignore to disable)")
    print("\n📋 Next Steps:")
    print(f"1. Set up environment: cd {tool_path} && uv sync")
    print(f"2. Create experiment configs in configs/")
    print(f"3. Test the tool: python scripts/run_experiment {tool_name} <config_path>")
    print(f"4. List available configs: python scripts/validate_setup.py --list-configs")
    print("\n💡 Tips:")
    print(f"- Check the example config: configs/example_{tool_name}.yaml")
    print(f"- Validate your setup: python scripts/validate_setup.py")
    print(f"- Run comprehensive test: python test_workflow.py")
...
Tools are automatically added to .gitignore by default to prevent accidental commits.
    ...
    parser.add_argument(
        "--entrypoint",
        help="Custom entrypoint (auto-detected if not specified)"
    )
    parser.add_argument(
        "--skip-validation",
        action="store_true",
        help="Skip post-setup validation"
    )
    parser.add_argument(
        "--gitignore",
        action="store_true",
    ...
    parser.add_argument(
        "--no-gitignore",
        action="store_true",
        help="Don't add tool to .gitignore"
    )
    ...
    try:
        ...
        tool_path = clone_and_add_submodule(args.tool_name, args.github_url, args.branch)
